...
public:
  ...
  FG_eval(VectorXd coeffs) { this->coeffs = coeffs; } // initialization

  typedef CPPAD_TESTVECTOR(AD<double>) ADvector; // rename
  ...
  void operator()(ADvector &fg, const ADvector &vars)
  {
     ...
     */
    // the part of the const based on the referenc state
    ...
    const double steer_weight = 100000;
    const double accel_weight = 10000;
    ...
    {
      fg[0] += steer_weight * CppAD::pow(vars[delta_start + t + 1] - vars[delta_start + t], 2);
      fg[0] += accel_weight * CppAD::pow(vars[a_start + t + 1] - vars[a_start + t], 2);
    }
...
std::vector<double> MPC::Solve(const VectorXd &state, const VectorXd &coeffs)
{
  ...
  {
    vars_lowerbound[i] = -std::numeric_limits<float>::max();
    vars_upperbound[i] = +std::numeric_limits<float>::max();
  }
  ...
  // Should be 0 besides initial state.**都是等式约束，上下限都设置为0，初始值的约束**
